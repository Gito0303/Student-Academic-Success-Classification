...
def predict_tabnet(model, input_data):
    prediction = model.predict(input_data)  
    return prediction
...
model_option = st.sidebar.selectbox(
    ...
    ['Feedforward Neural Network (FNN)', 'TabNet', 'Deep Neural Network (DNN)', 'Random Forest'],
    help="Pilih model yang ingin digunakan untuk prediksi (Feedforward Neural Network (FNN), TabNet, Deep Neural Network (DNN)) atau Random Forest."
)
...
st.sidebar.info(
    ...
    - Terdapat 4 (Tiga) Pilihan Model antara lain ; **Feedforward Neural Network (FNN)**, **TabNet**, **Deep Neural Network (DNN)** dan **Random Forest** untuk Klasifikasi Kesuksesan Akademik Mahasiswa.
...
if st.button("Prediksi"):
    ...
    elif model_option == 'TabNet':
        model = load_tabnet_model()
        prediction = predict_tabnet(model, np.array([input_data])) 
